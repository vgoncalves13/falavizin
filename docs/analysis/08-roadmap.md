...
| ✅ Limitar mapa — concluído na B018 | Endpoint público validado por viewport, resposta máxima de 200 e carregamento assíncrono | B003/B017 | P1 / M | Bounds inválidos e excesso de marcadores | Regressões de validação/limite, build e suíte de 224 testes passam | HTML e payload geográfico limitados |
| ✅ Consolidar cache — concluído na B019 | Dez chaves/TTL em serviço, stats fora da Blade e invalidação automática pós-commit | B017/B018 | P1 / M | Flush amplo em mutações frequentes | Regressão comprova atualização após cache preenchido; suíte de 225 testes passa | Home consistente sem queries na view |
| ✅ Alinhar ambiente de exemplo — concluído na B020 | Nome/locale, MySQL/Sail, file cache/session, public storage, database queue e Mailpit | Stack obrigatório | P1 / XS | Confundir defaults locais com produção | Valores não sensíveis refletem Compose e aplicação | Clone local previsível |
| ✅ Criar CI — concluído na B021 | Composer validate/audit, Pint, PHPUnit com MySQL 8, build e npm audit | B002/B020 | P1 / M | Divergência da imagem local | Comandos locais passam e workflow roda em push/PR | Regressões bloqueadas automaticamente |
| Baseline operacional restante | README, worker/scheduler, deploy, backup e restore | Decisões de deploy | P1 / S | Conhecimento tribal | Outra pessoa executa o runbook do zero | Operação reproduzível |
